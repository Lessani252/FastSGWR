...
class SGWR(GLM):
    ...
    def _build_wi(self, i, bw):
        ...
        if self.gwr:
            ...
            data_point = self.data.iloc[i, :].values  
            diff = np.abs(self.data.values - data_point)
            combined = np.mean(diff, axis=1)
            ws = np.exp(-combined ** 2)

            # # 1/26/2026
            # # 1. Extract the attribute values for the current regression point i
            # data_point = self.data.iloc[i, :].values  
            # stds = self.data.std(axis=0).values
            # stds = np.where(stds == 0, 1e-5, stds)
            # diff_standardized = np.abs(self.data.values - data_point) / stds
            # combined = np.mean(diff_standardized, axis=1)
            # ws = np.exp(np.log(0.5) * (combined ** 2))
            # ws = ws.reshape(-1, 1)
    ...
    def fit(self, ini_params=None, tol=1.0e-5, max_iter=20, solve='iwls',
        ...
        if solve.lower() == 'iwls':
            ...
            if lite:
                ...
                return SGWRResults(self, params, predy, S, CCT, influ, self.bt_value, self.data, tr_STS, w)
...
class SGWRResults(GLMResults):
    def __init__(self, model, params, predy, S, CCT, influ, bt_value, data, tr_STS=None, w=None):
